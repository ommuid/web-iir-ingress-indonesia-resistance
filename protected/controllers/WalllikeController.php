...
class WalllikeController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = OmmuWallLikes::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('OmmuWallLikes', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = 'Ommu Wall Likes';
		$this->pageDescription = $setting->meta_description;
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/wall_like/front_index',array(
	...
	public function actionView($id) 
	{
		...
		$this->pageTitle = 'View Ommu Wall Likes';
		$this->pageDescription = '';
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/wall_like/front_view',array(
			'model'=>$model,
		));
		/*
		$this->render('/wall_like/admin_view',array(
			'model'=>$model,
		));
		*/
	}	
	...
	{
		$model=new OmmuWallLikes;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['OmmuWallLikes'])) {
			$model->attributes=$_POST['OmmuWallLikes'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>OmmuWallLikes success created.</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', 'OmmuWallLikes success created.');
					//$this->redirect(array('view','id'=>$model->like_id));
					$this->redirect(array('manage'));
				}
			}
		}
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['OmmuWallLikes'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-ommu-wall-likes',
							'msg' => '<div class="errorSummary success"><strong>OmmuWallLikes success updated.</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', 'OmmuWallLikes success updated.');
					//$this->redirect(array('view','id'=>$model->like_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				OmmuWallLikes::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				OmmuWallLikes::model()->deleteAll($criteria);
			}
		}
	...
	public function actionPublish($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				...
				if($model->update()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-ommu-wall-likes',
						'msg' => '<div class="errorSummary success"><strong>OmmuWallLikes success published.</strong></div>',
					));
				}
			}
			...
			$this->pageDescription = '';
			$this->pageMeta = '';
			$this->render('/wall_like/admin_publish',array(
				'title'=>$title,
				'model'=>$model,
			));
		}
	...
	public function actionHeadline($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			$this->pageDescription = '';
			$this->pageMeta = '';
			$this->render('/wall_like/admin_headline');
		}
	}
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ommu-wall-likes-form') {
