...
if($render == 1) {
	if($data->another_relation->another_photo == '')
		$images = Utility::getTimThumb(Yii::app()->request->baseUrl.'/public/daop/daop_default.png', 100, 100, 1);
	else
		$images = Utility::getTimThumb(Yii::app()->request->baseUrl.'/public/daop/another/'.$data->another_relation->another_photo, 100, 100, 1); 
	if($data->another_relation->city_id != 0)
		$title = $data->another_relation->another_name.', '.$data->another_relation->city_relation->city;
	else
		$title = $data->another_relation->another_name;
?>
